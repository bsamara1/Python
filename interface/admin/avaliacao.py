...
class AvaliacaoPage(ctk.CTkFrame):
    ...
    def avaliar_com_prolog(self, tipo_bolsa, media, rendimento):
        ...
        try:
            ...
            # CORREÇÃO: Removido o bloco 'with' que causava o erro de context manager
            resultado = list(prolog_local.query(query_str))
    ...
    def carregar_dados_combos(self):
        """Carrega os dados das bolsas e dos estudantes com todas as colunas necessárias"""
        try:
            conn = conectar()
            cursor = conn.cursor()

            # 1. Carregar Estudantes
            cursor.execute("SELECT id, nome, media, rendimento FROM estudantes ORDER BY nome ASC")
            dados_estudantes = cursor.fetchall()
            self.lista_estudantes = dados_estudantes 
            
            nomes_estudantes = [f"{id_est} - {nome}" for id_est, nome, _, _ in dados_estudantes]
            if hasattr(self, 'combo_estudante'):
                if nomes_estudantes:
            ...
            # 2. Carregar Bolsas (CORRIGIDO: 'Ativo' em vez de 'Ativa')
            cursor.execute("SELECT id, nome, tipo FROM bolsas WHERE estado = 'Ativo' ORDER BY nome ASC")
            dados_bolsas = cursor.fetchall()
            self.lista_bolsas = dados_bolsas 
            
            nomes_bolsas = [nome for _, nome, _ in dados_bolsas]
            if hasattr(self, 'combo_bolsa'):
                if nomes_bolsas:
    ...
    def avaliar_candidatura(self):
        """Executa a avaliação corrigindo a busca por ID e o desempacotamento"""
        estudante_sel = self.combo_estudante.get()
        bolsa_sel = self.combo_bolsa.get()

        # Validação robusta contra qualquer string de placeholder ou erro
        if (estudante_sel in ["Selecione um Estudante", "Nenhum estudante cadastrado", ""] or 
            bolsa_sel in ["Selecione uma Bolsa", "Nenhuma bolsa ativa", "Nenhuma bolsa carregada", ""]):
            messagebox.showwarning("Campos Incompletos", "Por favor, selecione um estudante e uma bolsa válidos para avaliar.")
            return

        try:
            id_est_sel = int(estudante_sel.split(" - ")[0])
        except ValueError:
            messagebox.showerror("Erro", "Formato de estudante inválido selecionado.")
            return

        estudante_dados = next((e for e in self.lista_estudantes if e[0] == id_est_sel), None)
        bolsa_dados = next((b for b in self.lista_bolsas if b[1] == bolsa_sel), None)

        if not estudante_dados or not bolsa_dados:
            messagebox.showerror("Erro", "Erro ao recuperar dados armazenados para avaliação.")
            return
    ...
    def criar_interface(self):
        ...
        container_colunas.grid_columnconfigure(0, weight=4, uniform="coluna_av") 
        container_colunas.grid_columnconfigure(1, weight=5, uniform="coluna_av") 
