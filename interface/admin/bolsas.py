...
class BolsasPage(ctk.CTkFrame):
    """Página de Gestão de Bolsas com layout moderno e idêntico ao design do dashboard"""

    def __init__(self, master):
        # Fundo cinza claro idêntico ao do dashboard original (#F4F6FB)
    ...
    def criar_interface(self):
        # -------------------------------------------------------------
        # 1. CABEÇALHO (Título e Botão de Ação Superior)
        # -------------------------------------------------------------
        ...
        self.entry_pesquisa = ctk.CTkEntry(
            frame_filtros, 
            placeholder_text="🔍 Pesquisar por nome, ID ou tipo...",
            font=("Segoe UI", 13),
            fg_color="white",
            border_color="#E5E7EB",
            height=40,
        ...
        # -------------------------------------------------------------
        # 3. CARD CONTAINER PRINCIPAL DA TABELA
        # -------------------------------------------------------------
    ...
    def atualizar_tabela(self):
        # Limpar área interna da tabela anterior
        for widget in self.card_conteudo.winfo_children():
            widget.destroy()

        # Configuração de Colunas Proporcionais
        ...
        if not self.bolsas_filtradas:
            lbl_vazio = ctk.CTkLabel(
                self.card_conteudo, 
                text="Nenhuma bolsa ou edital encontrado.", 
                font=("Segoe UI", 14), 
                text_color="#9CA3AF"
            )
        ...
        for row_idx, b in enumerate(self.bolsas_filtradas, start=2):
            ...
            ctk.CTkLabel(self.card_conteudo, text=b['tipo'], font=("Segoe UI", 13), text_color="#4B5563", anchor="w").grid(row=row_idx*2, column=2, padx=20, pady=padd_y, sticky="w")
            
            valor_formatado = f"{b['valor']:,}$".replace(",", ".") if isinstance(b['valor'], (int, float)) else f"{b['valor']}"
            ...
            btn_edit = ctk.CTkButton(
                frame_acoes, text="✏️", width=30, height=30, fg_color="#F3F4F6", 
            ...
            btn_del = ctk.CTkButton(
                frame_acoes, text="🗑️", width=30, height=30, fg_color="#FEE2E2", 
    ...
    def abrir_formulario_bolsa(self, dados_bolsa=None):
        """Abre uma janela pop-up para Adicionar ou Editar dados de uma Bolsa"""
        janela_form = ctk.CTkToplevel(self)
        janela_form.title("Formulário de Bolsa")
        janela_form.geometry("450x550")
        janela_form.grab_set()  # Foca a atenção apenas nesta janela
        ...
        # Container interno para organização
        ...
        # Preencher os dados caso seja uma Edição
        if dados_bolsa:
            entry_nome.insert(0, str(dados_bolsa['nome']))
            entry_tipo.insert(0, str(dados_bolsa['tipo']))
            entry_valor.insert(0, str(dados_bolsa['valor']))
            combo_estado.set(str(dados_bolsa['estado']).capitalize())
    ...
    def salvar_dados(self, nome, tipo, valor, estado, janela, id_bolsa=None):
        """Valida e processa a persistência dos dados no SQLite (INSERT/UPDATE)"""
        ...
        try:
            # Força o valor a ser tratado numericamente
        ...
        try:
            ...
            # Fecha o formulário e atualiza a exibição em tempo real
            janela.destroy()
            self.carregar_bolsas()
            self.filtrar_dados()
